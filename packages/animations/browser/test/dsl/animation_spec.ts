 ...
 */
import {AUTO_STYLE, AnimationMetadata, AnimationMetadataType, animate, group, keyframes, query, queryAll, sequence, style, ɵStyleData} from '@angular/animations';
    ...
    describe('keyframe building', () => {
      ...
      describe('query()', () => {
        ...
        it('should throw an error when an animation query returns zero elements', () => {
          ...
          const steps2 = [queryAll(
              'fakeSomethings', [style({opacity: 0}), animate(1000, style({opacity: 1}))])];

          expect(() => { invokeAnimationSequence(rootElement, steps2); })
              .toThrowError(
                  /`queryAll\("fakeSomethings"\)` returned zero elements\. \(Use `queryAll\("fakeSomethings", \{ optional: true \}\)` if you wish to allow this\.\)/);
        });
